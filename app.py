...
# ── Column name keywords that indicate dollar / currency values ───────────────
CURRENCY_KEYWORDS = {
    "revenue", "cogs", "profit", "cost", "sales", "income",
    "turnover", "margin", "freight", "price", "amount", "earnings",
}
PCT_KEYWORDS = {"pct", "percent", "margin_pct", "rate"}
KG_KEYWORDS  = {"kg", "weight", "drykg"}


def format_value(col_name: str, value: float) -> str:
    """Return a human-friendly formatted string based on column name heuristics."""
    col = col_name.lower()
    if any(k in col for k in PCT_KEYWORDS):
        return f"{value:,.2f}%"
    if any(k in col for k in KG_KEYWORDS):
        return f"{value:,.0f} KG"
    if any(k in col for k in CURRENCY_KEYWORDS):
...
def format_df_columns(df):
    """Apply currency/pct formatting to numeric columns in a dataframe for display."""
    df_display = df.copy()
    for col in df_display.select_dtypes(include="number").columns:
        ...
        elif any(k in col_lower for k in KG_KEYWORDS):
            df_display[col] = df_display[col].apply(lambda v: f"{v:,.0f} KG" if pd.notna(v) else "")
        elif any(k in col_lower for k in CURRENCY_KEYWORDS):
            df_display[col] = df_display[col].apply(
                lambda v: (
                    f"${v / 1_000_000:,.2f}M" if pd.notna(v) and abs(v) >= 1_000_000
                    else f"${v:,.0f}" if pd.notna(v) and abs(v) >= 1_000
...
SEMANTIC_MODEL_STAGE = "@CORTEX_DB.AGENTS.MY_STAGE/my_model.yaml"
CORTEX_ANALYST_URL   = (
...
def get_jwt_token() -> str:
    ...
    Snowflake JWT format requirements:
    - account identifier: uppercase, only the account locator portion
      e.g. "NEA23255.east-us-2.azure" → "NEA23255"
    - qualified username: ACCOUNT.USER both uppercase
    - iss claim: ACCOUNT.USER.SHA256:fingerprint
    - sub claim: ACCOUNT.USER
    """
    ...
    # Snowflake JWT needs only the account locator — strip region and cloud
    # "NEA23255.east-us-2.azure" → "NEA23255"
    account_locator = SNOWFLAKE_ACCOUNT.split(".")[0].upper()
    qualified_user  = f"{account_locator}.{SNOWFLAKE_USER.upper()}"
...
def display_result(item: dict):
    ...
    if df.shape == (1, 1):
        ...
        label = df.columns[0]
        try:
            st.metric(label=label, value=format_value(label, float(value)))
        except Exception:
    ...
    # two columns, second numeric → bar chart
        ...
        st.dataframe(format_df_columns(df), use_container_width=True)
